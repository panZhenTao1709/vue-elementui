...
export default {
  ...
  methods: {
    ...
    add() {
      ...
      this.isAdd = true
      this.$refs.form.getParentRole()
      // this.$refs.form.form = {}  //每次增加都是重置为空
    },
    edit() {
       this.$refs.form.getParentRole()
      ...
      console.log(role)
       this.id = role.id
      this.$refs.form.form = { name: role.name, roleType: role.roleType, isDisable: role.isDisable, id: role.id,parentId:role.parentId}
    },
    del() {
      if(this.sup_this.mutiSelection.length == 0){
        this.$message({
