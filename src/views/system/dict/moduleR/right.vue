<template>
  <el-card class="box-card" shadow="never">
    ...
    <div slot="header" class="clearfix">
      <span class="role-span">{{$t('table.detailsOfTheDictionary')}}</span>
    </div>
    <el-table :data="data" highlight-current-row size="small" style="width: 100%;" border @row-click="handleCurrentChange">
      <el-table-column prop="label" :label="this.$t('table.theDictionaryLabels')"/>
      <el-table-column prop="value" :label="this.$t('table.theFieldValues')"/>
      <el-table-column prop="sort" :label="this.$t('table.sort')"/>
    ...
    methods: {
      beforeInit() {
        ...
        if (this.query.parentId) {
          this.params['parentId'] = this.query.parentId
        }
        return true
      },
      subDelete(id) {
